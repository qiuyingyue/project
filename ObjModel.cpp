			...
			for (int k = 0; k < 4; k++){
				...
				if (k == 3){
					if (s[0] >= '0' && s[0] <= '9'){
						...
						fseek(file, -(LONG)s.size()-2, SEEK_CUR);
						break;
					}
				}
...
bool ObjModel::loadMTL(){
    ...
    while(true){
        ...
        if (strcmp(lineHeader, "newmtl") == 0 ){
			...
			temp_mtl.textid = textpoint++;//###
            cout << ch << endl;
            string texPath = filePath + "/" + ch;
            strcpy(ch, texPath.c_str());
			//soil_texture(ITEM, texture, ch);
			soil_texture(temp_mtl.textid, textureObjects[temp_mtl.textid], ch);//###
        }
        else{
            // Probably a comment, eat up the rest of the line
            char stupidBuffer[1000];
    ...
    for (int i = 0; i < groups.size(); i++){
        ...
        if (!group.material.empty()){
            ...
            if (material.texture < 1000){
				...
				//textureObjects[ITEM] = material.texture;
				glActiveTexture(GL_TEXTURE0 + ITEM);
                glBindTexture(GL_TEXTURE_2D, material.texture);  //选择纹理texture[0]
            }
			if (material.textid >= ITEM){
				glActiveTexture(GL_TEXTURE0 + material.textid);
				glBindTexture(GL_TEXTURE_2D, textureObjects[material.textid]);
				glUniform1i(glGetUniformLocation(programs[currentShader], "tex"), material.textid);
			}
		}
		//glUniform1i(glGetUniformLocation(programs[currentShader], "tex"), FLOOR);//####
        glPushMatrix();
